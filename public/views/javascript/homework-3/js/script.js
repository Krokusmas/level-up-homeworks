...
"" + 1 + 0 // 10. Один из элементов строка, поэтому остальные преобразуются в строку и складываются в строку.
"" - 1 + 0 // -1. Только арфметический оператор + является строковым, остальные не преобразуют операнды в строки. Поэтому происходит преобразование к числу, а пустая строка равна 0.
...
4 + 5 + "px" // "9px". Арифметические операции выполняются слево направо. Сначала получается сумма чисел 9, затем из-за того, что один операнд строка и стоит строковой оператор +, происходит преобразование числа к строке.
...
"  -9\n" + 5 // "-9\n5". Т.к. один из элемннтов строка, строковый оператор плюс преобразует 5 в строку и прибавляет к первой строке.
...
null == "\n0\n" // false. null сравнивается с непустой строкой. При приведении к логическим значениям null = false, а непустая строка даёт true. Поэтому они не равны.
